...
- (void)addUI{
    ...
    [self.view bringSubviewToFront:_pointImageView];
...
- (IBAction)closeButtonClick:(UIButton *)sender {
    ...
    if (_resultLabel.text) {
        self.closeScanBarCodeValue(_resultLabel.text);
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (metadataObjects.count > 0) {
        ...
        self.returnScanBarCodeValue(string);
        _closeButton.hidden = NO;
        _pointImageView.hidden = YES;
        _resultLabel.text = string;
        _resultLabel.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.6];
